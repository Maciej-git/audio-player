/*Audio Player Mini-Project

Final program for Embedded System Essentials with Arm course. 
...
BufferedSerial pc(USBTX, USBRX, 9600); // Extend communication with user, through the PC screen
...
// Names and numbers to present while in "Ready" state, giving the user information about available songs  

const char *titles[] = {"0: Oranges & Lemons", "1: Cielito Lindo", "2: Malaika", "3: Guten Abend",  
                        "4: Yankee Doodle", "5: Rasa Sayang", "6: Walzing Matilda", "7: Alouetta"}; 
...
//Waiting for song 
    ...
    while(1){ 
        ...
        printf("Choose a song and then press play\r\n"); 
...
Thread thread4; // Presents available songs while in "Ready" state

void Show_songs(const void *args){
        ...
        if (ready_led) {
            ...
            for (i = 0; i < 8; i++) {
                if (!playing) {
                    ...
                    lcd_mutex.lock();      
                    clr_lcd();   //Clear the LCD
                    print_lcd(titles[i]);   //display song name
                    lcd_mutex.unlock();
                }
                 
                thread_sleep_for(1000); // One sec scroll interval
            }
        }
    }
}
...
int main() { 
    ...
    thread4.start(callback(Show_songs, &ready_led)); 
